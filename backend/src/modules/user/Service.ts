...
class UserService {
  ...
  getById = async (userId: string, fields: string): Promise<IUser | null> => {
    const result: IUser | null = await this.userRepository.getById(userId, fields);
    return result;
  };
  ...
  static verifyPassword = async (
    ...
    const result: boolean = await bcrypt.compare(currentUser.password, existingUser.password);
    return result;
  };

  static generateLoginToken = async (existingUser: IUser, jwtSecret: string): Promise<string> => {
